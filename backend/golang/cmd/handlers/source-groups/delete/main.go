...
type DeleteSourceGroupHandler struct {
...
func NewDeleteSourceGroupHandler() (*DeleteSourceGroupHandler, error) {
	...
	return &DeleteSourceGroupHandler{db: db}, nil
}

func (h *DeleteSourceGroupHandler) Handle(ctx context.Context, event events.APIGatewayProxyRequest) (events.APIGatewayProxyResponse, error) {
	log.Printf("Delete source group request started")
	...
	log.Printf("Delete source group %s for user %s, account %s", groupId, userID, accountID)
	...
	// First verify the source group exists and belongs to user
	...
	// Check if group has sources - prevent deletion if it has sources
	if sourceGroup.SourceCount > 0 {
		return events.APIGatewayProxyResponse{
			...
			Body: `{"success": false, "error": "Cannot delete source group that contains sources. Remove all sources first."}`,
		}, nil
	}
	...
	if err != nil {
		log.Printf("Failed to delete source group %s: %v", groupId, err)
		return events.APIGatewayProxyResponse{
			StatusCode: 500,
			Headers: map[string]string{
				"Access-Control-Allow-Origin":  "*",
			...
			Body: `{"success": false, "error": "Failed to delete source group"}`,
		}, nil
	}
	...
	log.Printf("Deleted source group %s", groupId)
	return events.APIGatewayProxyResponse{
		StatusCode: 200,
		Headers: map[string]string{
			"Access-Control-Allow-Origin":  "*",
...
func main() {
	handler, err := NewDeleteSourceGroupHandler()
	if err != nil {
		log.Fatalf("Failed to create delete source group handler: %v", err)
	}
