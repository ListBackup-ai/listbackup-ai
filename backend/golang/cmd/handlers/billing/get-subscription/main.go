...
type GetSubscriptionResponse struct {
	...
	Plan         *types.BillingPlan  `json:"plan"`
...
func handler(ctx context.Context, event events.APIGatewayProxyRequest) (events.APIGatewayProxyResponse, error) {
	log.Printf("Get subscription request: %+v", event)
	...
	plan, err := billingService.GetBillingPlan(ctx, subscription.PlanID)
	if err != nil {
		log.Printf("Failed to get billing plan: %v", err)
		return util.CreateErrorResponse(500, "Failed to get plan details")
	}

	// Get current month's usage
	currentPeriod := subscription.CurrentPeriodStart.Format("2006-01")
	usage, err := billingService.GetUsageForPeriod(ctx, accountID, currentPeriod)
	if err != nil {
		log.Printf("Failed to get usage data: %v", err)
		usage = []types.UsageRecord{} // Return empty usage instead of failing
	}
