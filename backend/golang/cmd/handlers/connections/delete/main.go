...
func NewDeleteConnectionHandler() (*DeleteConnectionHandler, error) {
...
func (h *DeleteConnectionHandler) Handle(event events.APIGatewayProxyRequest) (events.APIGatewayProxyResponse, error) {
	...
	log.Printf("Delete connection request: %s for user %s, account %s", connectionId, userId, accountId)
	...
	// TODO: Check if connection is being used by any sources
	// For now, we'll allow deletion but should implement dependency checking

	// Delete the connection
	err = h.db.DeleteItem(database.PlatformConnectionsTable, key)
	if err != nil {
		log.Printf("Failed to delete connection %s: %v", connectionId, err)
		return response.InternalServerError("Failed to delete connection"), nil
	}

	log.Printf("Deleted connection %s", connectionId)
...
func main() {
	handler, err := NewDeleteConnectionHandler()
	if err != nil {
		log.Fatalf("Failed to create delete connection handler: %v", err)
	}
